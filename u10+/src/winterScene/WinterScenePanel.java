...
public class WinterScenePanel extends JPanel implements Runnable {
	...
	public WinterScenePanel() {
		...
		for(int i = 0; i <= 49; i++) {
			shapes.add(new FancySnowFlake((int) (Math.random() * 799), (int) (Math.random() * 400), (int) (Math.random() * 25), (int) (Math.random() * 25), Color.white, 0, (int) (Math.random() * 4) + 1));
		}
		// instantiate a snowman
		sMan = new SnowMan(266, 350, 100, 100);
		t = new Tree(50, 375, 90, 90, Color.GREEN);
	...
	public void paint(Graphics window) {
		...
		// screen
		for(AbstractShape k: shapes) {
			k.moveAndDraw(window);
			if(k.getYPos() >= super.getHeight())
				k.setYPos(0);
		}
	}
