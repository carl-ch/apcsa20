...
public class ElevensBoard extends Board {
	...
	public boolean isLegal(List<Integer> selectedCards) {
		...
		if ((selectedCards.size() == 2 || selectedCards.size() == 3)
				&& (containsPairSum11(selectedCards) || containsJQK(selectedCards))) {
			return true;
		}
	...
	public boolean anotherPlayIsPossible() {
		...
		if (containsPairSum11(nums) || containsJQK(nums)) {
			return true;
		}
	...
	private boolean containsPairSum11(List<Integer> selectedCards) {
		...
		for (int i = 0; i < selectedCards.size() - 1; i++) {
			for (int i2 = i + 1; i2 < selectedCards.size(); i2++) {
				if (cardAt(selectedCards.get(i)).pointValue() + cardAt(selectedCards.get(i2)).pointValue() == 11) {
					return true;
				}
			}
		}
